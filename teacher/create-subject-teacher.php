...
else
{
    if(isset($_POST['submit']))
    {
        ...
        $result_b=mysqli_query($conn, "SELECT * FROM  tblsubjects WHERE SubjectName= '$subjectname' OR SubjectCode='$subjectcode';");
        if(mysqli_num_rows($result_b)>0)
        {
            echo "<script>alert('Subject/Subject ID already exists');</script>";
            echo "<script type='text/javascript'> window.document.location = '../teacher/create-subject-teacher.php'; </script>";
        }
        else
        {
